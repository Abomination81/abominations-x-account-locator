...
Abominations X Account Locator places X's public **Account based in** disclosure directly on ordinary posts, quoted posts, and follower/following lists. The badge color is customizable and defaults to Abomination green (`#39ff14`). Version 0.8 excludes the signed-in account from location lookups and badges, and gives `@Abomination81` the extension-only custom location `XANADU` for everyone else.
...
1. Download and unzip `abominations-x-account-locator-v0.8.0.zip` from the installation page.
